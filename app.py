    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if "CUDA out of memory" in str(e):
                print("CUDA OOM detected, forcing exit to trigger container restart.")
                sys.stdout.flush()
                os._exit(1)
            raise
...
def set_or_reset_timer(session_id, timeout_seconds=900):
...
def initialize_video():
    ...
    dicom_filenames = sorted(
        dicom_filenames, key=lambda fname: sort_key(pydicom.dcmread(fname))
    )
    ...
    for i, s in enumerate(slices):
        ...
        if ww and wl:
            ...
            img2d = (
                (img2d - min_val) / (max_val - min_val) * 255
...
def convert_masks_to_nii(video_segments, n_frames):
    """
    Convert the ropagated masks to a NIfTI (.nii.gz) file from video segments.
    ...
    )  # Get masks from the first frame
    ref_img_shape = first_frame_masks[
        next(iter(first_frame_masks))
    ].shape  # Get the shape of the first mask
    ...
    for out_frame_idx, masks in video_segments.items():
        for obj_id, mask in masks.items():
            ...
            combined_mask[out_frame_idx] = np.where(
                mask > 0, obj_id + 1, combined_mask[out_frame_idx]
            )

    # Create a Nifti1Image with the combined mask
    affine = np.eye(
        4
    )  # Identity affine matrix, replace with actual affine if available
