...
def initialize_video():
    ...
    try:
        with open(zip_file_path, 'rb') as f:
            ...
            response = requests.post(f'{server_url}/initialize_video', files=files, data=wwwl)
...
def create_overlay_video(session_id, nii_files_dir):
    nifti_files = sorted([f for f in os.listdir(nii_files_dir) if f.endswith('.nii.gz')])
    overlay_images = []
    ...
    for i, nii_filename in enumerate(nifti_files):
        ...
        nii_data = np.transpose(nii_data, (1, 2, 0))  # Transpose for [height, width, slices]
        ...
        for slice_idx in range(num_slices):
            ...
            output_overlay_path = os.path.join(output_images_dir, f'overlay_{slice_idx}.png')
